...
本版本包含了多个安全相关的功能，比如用户命名空间的支持、SELinux 挂载容器组的优化、CSI NodeExpansion 支持 Secrets 等。除此之外，为了提高集群管理员和应用管理员的体验，Job 增加了不重试选项，密钥管理服务 KMS v2 优化，kubelet 支持获取容器组的检查点以更好地进行事故分析。更多重要功能会在下文详细介绍。
...
此外，该功能的监控方法之前版本默认使用 du 来计算磁盘占用情况，在 v1.25 中，如果磁盘支持 fsquota，磁盘使用情况将使用 fsquota 进行计算，性能上会有很大的提升。
