...
2026 年 4 月 23 日（北京时间周四），Kubernetes v1.36 正式发布。此版本包含 69 项改进。在这些改进功能中，有 18 项已晋升为 Stable，有 24 项进入 Beta 阶段，有 25 项为 Alpha 阶段，2 项进入废弃阶段。
...
![Kubernetes v1.36 Logo](./k8s-v1.36.png)
...
在富士山脚下，Stella（左）与 Nacho（右）两只猫静静而立，颈间佩戴着 Kubernetes 船舵徽章，象征着日本神社中成对守护的“狛犬”。成双成对，因为守护从来不是孤独完成的。Stella 与 Nacho 也代表着一个更为庞大的群体：SIG 与工作组、维护者与评审者、文档与博客与翻译的贡献者、发布团队、迈出第一步的新手，以及一季又一季持续回归的长期贡献者。Kubernetes v1.36，一如既往，是由无数双手共同托举起来的。
